...
function Factory(options) {
	...
	function moduleOf(className, constructor) {
		...
		throw Error('factory error : could not find module for '+className + ' required by '+(constructor && constructor.name) + ' - dependencies : ' + JSON.stringify(Object.keys(options.dependencies)));
	};
	...
	function get(constructor, parent, name, module, argsMap, opts) {
		var object = global[prefix] && global[prefix][id] && global[prefix][id][module] && global[prefix][id][module][name];
	...
	function create(constructor, parent, name, argsMap, opts) {
		...
		if(typeof constructor === 'string') {
			constructor = requireArg(constructor, parent);
		}
